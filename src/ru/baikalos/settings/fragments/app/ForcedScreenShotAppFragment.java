...
@Keep
public class ForcedScreenShotAppFragment extends AppSwitchPreferenceFragmentBase {
    ...
    protected int getTitleStringId() {
        return R.string.app_forced_screen_shot_title;
    }

    @Override
    protected int getCategoryTitleStringId() {
        return R.string.app_forced_screen_shot_category_title;
    }


    @Override
    protected boolean shouldIncludeApp(ApplicationInfo info, BaikalAppProfile profile) {
        return profile != null && (profile.mOverride & BaikalAppProfile.BAIKAL_OVERRIDE_FORCED_SCREENSHOT) != 0;
    }
    ...
    protected boolean onCheckChanged(ApplicationInfo info, BaikalAppProfile profile, boolean newValue) {
        ...
        if (!newValue) {
            profile.mOverride &= ~BaikalAppProfile.BAIKAL_OVERRIDE_FORCED_SCREENSHOT;
        } else {
            profile.mOverride |= BaikalAppProfile.BAIKAL_OVERRIDE_FORCED_SCREENSHOT;
        }
